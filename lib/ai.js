/* ════════════════════════════════════════════════════════════════
   SETU — Pure Groq AI Engine (Server-Side)
   - 100% Groq: Llama 3.1 8B (Fast Ingest) + Llama 3.3 70B (Deep Reasoning)
   - 1. Classify   → Groq Llama 3.1 8B (else explainable keyword engine)
   - 2. Dedup      → 2-Stage Hybrid: Spatial + Token filter + Groq Arbiter
...
const GROQ_MODEL_DEEP = process.env.GROQ_MODEL_DEEP || "groq/compound";
...
export async function classify(text) {
  ...
  const groqResult = await callGroq({
    ...
    messages: [
      {
        role: "system",
        content: `You are the AI triage coordinator for the Government of Jharkhand civic grievance portal (SETU).
...
/* ── 6. MULTILINGUAL AUTO-DESCRIPTION (Groq Llama 3.3 70B) ── */
export async function suggestDescription(title, language = "auto") {
  ...
  const result = await callGroq({
    ...
    messages: [
      {
        role: "system",
        content: `You are an assistant for SETU, the civic grievance portal of Jharkhand, India.
...
/* ── 7. AI UNIVERSITY PROPOSAL GENERATOR (Groq Llama 3.3 70B) ── */
